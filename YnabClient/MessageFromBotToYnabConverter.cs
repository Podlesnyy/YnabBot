...
public sealed class MessageFromBotToYnabConverter : IMessageReceiver, IDbSaver
{
        ...
        Oauth oauth)
    {
        ...
        users = dbContext.Users.Include(static item => item.BankAccountToYnabAccounts)
            .ThenInclude(static item => item.YnabAccount).Include(static item => item.DefaultYnabAccount)
            .Include(static item => item.Access).ToList();
    ...
    {
        return dicYnabUsers.ContainsKey(replyInfo.UserId)
    ...
    private Domain.User GetDbUser(string messengerUserId)
    {
        ...
        {
            MessengerUserId = messengerUserId, BankAccountToYnabAccounts = new List<BankAccountToYnabAccount>(),
            DefaultYnabAccount = new YnabAccount(), Access = new YnabAccess()
        };
