...
public sealed class TelegramBot(IConfiguration configuration) : IMessageSender
{
    ...
    public async Task SendOptions(ReplyInfo replyInfo, string message, List<string> options)
    {
        ...
        {
            logger.Info(
                $"Reply with keyboard in chat {replyInfo.ChatId} message: {string.Join(";", options)} on {replyInfo.MessageId}");
    ...
    public async Task SendMessage(ReplyInfo replyInfo, string message)
    {
        ...
        {
            logger.Info(
                $"Reply in chat {Convert.ToInt64(replyInfo.ChatId)} message: {message} on {Convert.ToInt32(replyInfo.MessageId)}");
        ...
        try
        {
            if (message.Document == null)
            {
                ...
                messageReceiver.OnMessage(
                    new ReplyInfo(message.From!.Id.ToString(), message.Chat.Id.ToString(),
                        message.MessageId.ToString()), message.Text);
            }
            else
            {
                logger.Info(
                    $"Received a document message in chat {message.Chat.Id} message: {message.Document.FileName}");
